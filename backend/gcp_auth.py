...
def resolve_google_application_credentials() -> Optional[str]:
    """
    Normalize and export a usable Google credentials path when one is available.

    Search order:
      1. GOOGLE_APPLICATION_CREDENTIALS from the environment
      2. Common local key filenames in backend/
    """
    candidates = []
