...
def check_prices():
    ...
    try:
        ...
        print("SCHEDULER_FATAL_ERROR:", error)
...
def _fetch_flight_offers_with_retry(amadeus_client, target: dict) -> list:
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            last_error = error
            status_code = _safe_int(getattr(getattr(error, "response", None), "status_code", None), 500)
...
def _build_snapshot(target: dict, offers: list, fetched_at: str, ingest_run_id: str, raw_gs_uri: str) -> dict:
    prices = [p for p in (_extract_price(offer) for offer in offers) if p is not None]
...
def _upload_raw_payload(bucket, route_key: str, ingest_run_id: str, fetched_at: str, payload: dict) -> str:
...
def ingest_flights():
    ...
    for target in targets:
        ...
        try:
            ...
            _write_ingestion_results(route_key=route_key, snapshot=snapshot, history_doc_id=ingest_run_id)
